...
public class Union {
    public static void podstr(Scanner sc) {
        try (Connection connection = MysqlConfig.getConnection()) {
            ...
            int Num2;
            while (true) {
                System.out.print("Введите индекс символа, после которого хотите вставить вторую строку: ");
                if (sc.hasNextInt()) {
                    Num2 = sc.nextInt();
            ...
            try (PreparedStatement psInsert = connection.prepareStatement(printSQL)) {
                ...
                while (rs.next()) {
                    String updateSQL = "UPDATE " + tbName + " SET Sovm = ? WHERE Str1 = ?";
                    try (PreparedStatement psUpdate = connection.prepareStatement(updateSQL)) {
                        String num = rs.getString("Str1"); StringBuffer nm = new StringBuffer(rs.getString("Str2"));
                        nm.insert(Num2, num);
                        psUpdate.setString(1, nm.toString());
                        psUpdate.setString(2, num);
                        int rowsAffected = psUpdate.executeUpdate();
                        if (rowsAffected > 0) {
                            System.out.println("Объединённые строки: " + nm);
                        }  else {
